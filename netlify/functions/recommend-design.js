...
exports.handler = async function(event, context) {
    ...
    if (!process.env.OPENAI_API_KEY) {
        return {
            statusCode: 500,
            body: JSON.stringify({ recommendation: "치명적 오류: 서버에 OPENAI_API_KEY가 설정되지 않았습니다." })
        };
    }
    ...
    try {
        ...
        if (!group) {
            throw new Error("해당 좌표의 그룹을 찾을 수 없습니다.");
        }
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ recommendation: 'AI 추천 생성 중 오류가 발생했습니다: ' + error.message })
        };
    }
};